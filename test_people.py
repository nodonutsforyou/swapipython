...
def test_positive_type(id):
    ...
    for string_field in expected_string_fields:
        ...
        assert isinstance(person[string_field], str), f'json does not have expected type for a field {string_field}:\n{str(person)}'
    for list_field in expected_list_fields:
        assert list_field in person, f'json does not contain expected field {string_field}:\n{str(person)}'
        assert isinstance(person[list_field],list), f'json does not have expected type for a field {string_field}:\n{str(person)}'
...
def test_paginations():
    ...
    assert isinstance(size, int), f'Incorrect value in count field ({str(size)})'
    ...
    assert collected_items == size, 'Unexpected number of collected items across all pages'
